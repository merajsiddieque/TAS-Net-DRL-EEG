...
parser.add_argument('--shared_params', action='store_true', default=False, help='Share Network1 parameters')
parser.add_argument('--combine_method', type=str, default='union', choices=['average', 'union'], help='Combine method')
parser.add_argument('--lambda_div', type=float, default=0.1, help='Diversity penalty coefficient')
...
if __name__ == '__main__':
    if args.training:
        ...
        if args.shared_params:
            net1 = ClassifierGNN(in_features=args.n_feature, edge_features=args.edge_features, out_features=args.n_feature, device=DEVICE)
            net1 = net1.to(DEVICE)
            Networks1 = [net1] * args.num_agents
            parameter_list.append({'params': net1.parameters(), 'lr_mult': 10, 'decay_mult': 2})
            print("Model size Net1 (Shared): {:.5f}M".format(sum(p.numel() for p in net1.parameters()) / 1000000.0))
        else:
            for i in range(args.num_agents):
                net1 = ClassifierGNN(in_features=args.n_feature, edge_features=args.edge_features, out_features=args.n_feature, device=DEVICE)
                net1 = net1.to(DEVICE)
                Networks1.append(net1)
                parameter_list.append({'params': net1.parameters(), 'lr_mult': 10, 'decay_mult': 2})

        for i in range(args.num_agents):
            net2 = DSN(in_dim=args.n_feature, hid_dim=args.hid_dim, num_layers=1, cell='gru')
            net2.apply(weights_init)
            net2 = net2.to(DEVICE)
            Networks2.append(net2)
            parameter_list.append({'params': net2.parameters(), 'lr_mult': 10, 'decay_mult': 2})
        ...
        for net1 in Networks1: net1.train()
        for net2 in Networks2: net2.train()
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            # --- Initialize CSV logging variables for this epoch ---
            epoch_div_penalties = []
            epoch_agent_similarities = []
            ...
            for idx in idxs:
                ...
                for a_idx in range(args.num_agents):
                    net1 = Networks1[a_idx]
                    net2 = Networks2[a_idx]
                    local_graphs0 = None
                    ...
                    agent_seq = seq_graph0.unsqueeze(dim=0)
                    agent_sig_probs = net2(agent_seq)
                    all_sig_probs.append(agent_sig_probs)
                    all_seqs.append(agent_seq)
                
                # Combine outputs
                stacked_probs = torch.stack(all_sig_probs, dim=0)
                if args.combine_method == 'average':
                    combined_probs = torch.mean(stacked_probs, dim=0)
                else: # union
                    combined_probs, _ = torch.max(stacked_probs, dim=0)
                    
                cost += 0.01 * (combined_probs.mean() - 0.5) ** 2
                    
                combined_seq = torch.mean(torch.stack(all_seqs, dim=0), dim=0)
                
                # Diversity Penalty
                R_div = 0.0
                pairs = 0
                for a1 in range(args.num_agents):
                    for a2 in range(a1 + 1, args.num_agents):
                        sim = torch.nn.functional.cosine_similarity(all_sig_probs[a1].view(-1), all_sig_probs[a2].view(-1), dim=0)
                        R_div -= sim
                        pairs += 1
                if pairs > 0:
                    R_div = R_div / pairs
                else:
                    R_div = torch.tensor(0.0).to(DEVICE)
                
                epoch_div_penalties.append(R_div.item())
                epoch_agent_similarities.append(-R_div.item())

                # Backpropagate R_div properly by subtracting it from cost (maximizing R_div)
                cost -= (args.lambda_div * 0.01) * R_div

                # cost = Inconsistency_loss(sig_probs, sig_trans)
                m = Bernoulli(combined_probs)
                epis_rewards = []

                for episode in range(5):
                    actions = m.sample()
                    log_probs = m.log_prob(actions)
                    task_reward = compute_reward(combined_seq, actions)
                    reward = task_reward
                ...
                optimizer.zero_grad()
                cost.backward()
                for net1 in set(Networks1): torch.nn.utils.clip_grad_norm_(net1.parameters(), 5.0)
                for net2 in set(Networks2): torch.nn.utils.clip_grad_norm_(net2.parameters(), 5.0)
                optimizer.step()

            epoch_reward = np.mean([reward_writers[key][epoch] for key in train_keys])
            avg_div_penalty = np.mean(epoch_div_penalties)
            avg_agent_similarity = np.mean(epoch_agent_similarities)

            if (epoch + 1) % 1 == 0:
                print("epoch {}/{}\t reward {}\t  cost {}\t".format(epoch + 1,
                                                                    100, epoch_reward, cost))
            if (epoch + 1) % 1 == 0:
                Recall = evaluate(args, Networks1, Networks2, datasets, test_keys)
                print(Recall)
                
                # --- Save Metrics ---
                metric_file = os.path.join('logs', 'multi_agent_metrics.csv')
                os.makedirs('logs', exist_ok=True)
                file_exists = os.path.isfile(metric_file)
                with open(metric_file, mode='a', newline='') as f:
                    writer = csv.writer(f)
                    if not file_exists:
                        writer.writerow(['epoch', 'reward', 'diversity_penalty', 'agent_similarity', 'recall'])
                    writer.writerow([epoch + 1, epoch_reward, avg_div_penalty, avg_agent_similarity, Recall])
                    
                if Recall > best_recall:
                    best_recall = Recall
                    # save first agent
                    model_state_dict1 = Networks1[0].state_dict()
                    model_state_dict2 = Networks2[0].state_dict()
        ...
        with torch.no_grad():
            ...
            for key_idx, key in enumerate(test_keys):
                ...
                # Multi-agent Evaluation Loading Checkpoint logic
                all_sig_probs = []
                for a_idx in range(args.num_agents):
                    net1 = model1 # use loaded model1 (which could be list if we modified script farther down, but this block is generic placeholder in eval branch)
                    net2 = model2
                    # Note: The evaluation below in the test branch is deprecated as we updated evaluate.py, 
                    # but if executed linearly here, we also need to adapt it. 
                    # For simplicity, we just pass the sequence to the single loaded models.
