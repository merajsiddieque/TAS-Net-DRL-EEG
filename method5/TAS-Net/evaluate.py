...
def evaluate(args, Networks1, Networks2, dataset, test_keys):

    with torch.no_grad():
        for net1 in Networks1: net1.eval()
        for net2 in Networks2: net2.eval()
        all_features = None
        all_labels = None
        num_segments_trial = []
        ...
        for key_idx, key in enumerate(test_keys):
            ...
            for a_idx in range(args.num_agents):
                ...
                sig_probs = net2(seq_graph0)
                all_sig_probs.append(sig_probs)
            ...
            probs_importance = combined_probs.data.cpu().squeeze().numpy()
