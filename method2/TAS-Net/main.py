    ...
    if args.training:
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            for idx in idxs:
                ...
                logits, v = Network2(seq)

                # Actor-Critic Loss formulation
                y = F.gumbel_softmax(logits, tau=0.5, hard=False)
                actions = y[..., 1:]
                ...
                log_prob = torch.sum(y.detach() * log_p, dim=-1, keepdim=True)

                task_reward = compute_reward(seq, actions)
                sparsity_reward = - actions.mean()
                reward = task_reward + sparsity_reward
                ...
                reward_writers[key].append(reward.item())
            ...
            if (epoch + 1) % 1 == 0:
                print("epoch {}/{}\t reward {}\t  loss {}\t policy_loss {}\t value_loss {}\t entropy {}\t".format(
                    epoch + 1, 100, epoch_reward, loss.item(), mean_policy_loss, mean_value_loss, mean_entropy))
            if (epoch + 1) % 1 == 0:
                Recall = evaluate(args, Network1, Network2, datasets, test_keys)
                print(Recall)
                
                os.makedirs('logs', exist_ok=True)
                csv_path = 'logs/actor_critic_metrics.csv'
                file_exists = os.path.isfile(csv_path)
                with open(csv_path, 'a', newline='') as csvfile:
                    writer = csv.writer(csvfile)
                    if not file_exists:
                        writer.writerow(['epoch', 'policy_loss', 'value_loss', 'entropy', 'reward_mean', 'recall'])
                    writer.writerow([epoch+1, mean_policy_loss, mean_value_loss, mean_entropy, epoch_reward, Recall])
        ...
        with torch.no_grad():
            ...
            for key_idx, key in enumerate(test_keys):
                ...
                logits, v = model2(seq2seq)
                sig_probs = F.softmax(logits, dim=-1)[..., 1]
